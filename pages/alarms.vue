    ...
    <div class="row">
      <div class="col-sm-12">
        <card v-if="$store.state.devices.length > 0">
          <div slot="header">
            <h4 class="card-title">
              Crear una nueva Alarma {{ selectedWidgetIndex }}
            </h4>
          </div>
        ...
        <card v-else>
          Tu necesitas seleccionar un dispositivo para crear una Alarma!        </card>
      </div>
    </div>
        ...
        .then(res => {
          if (res.data.status == "success") {
            this.$notify({
              type: "success",
              icon: "tim-icons icon-check-2",
              message: "Bien! Alarma Actualizada."
            });
